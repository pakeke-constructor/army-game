...
test("lazy: iterate registers tracked component", function()
    local w = ECSWorld()
    assert(w.componentIndex.foo == nil)
    assert(not w.trackedComponents:has("foo"))
    w:iterate("foo")
    assert(w.componentIndex.foo ~= nil, "should create list on miss")
    assert(w.trackedComponents:has("foo"), "should track component")
end)

test("lazy: untracked components not indexed", function()
    local w = ECSWorld()
    local e = makeEntity({hp = 10, mp = 5})
    w:addEntity(e)
    w:update(0)
    assert(w.componentIndex.hp == nil, "hp not iterated, should not be indexed")
    assert(w.componentIndex.mp == nil)
    w:iterate("hp")
    assert(w.componentIndex.hp ~= nil)
    assert(w.componentIndex.mp == nil, "mp still not tracked")
end)

test("lazy: first iterate sees existing entities", function()
    local w = ECSWorld()
    local e = makeEntity({hp = 10})
    w:addEntity(e)
    w:update(0)
    assert(countIterate(w, "hp") == 1, "lazy build should scan entities")
end)

test("lazy: tracked component persists across updates", function()
    local w = ECSWorld()
    local e1 = makeEntity({hp = 10})
    w:addEntity(e1)
    w:update(0)
    w:iterate("hp")
    local e2 = makeEntity({hp = 20})
    w:addEntity(e2)
    w:update(0)
    assert(countIterate(w, "hp") == 2)
end)
